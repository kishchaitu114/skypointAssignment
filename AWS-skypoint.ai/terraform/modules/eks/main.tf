...
# IRSA role for healthcare-app pods (Secrets Manager CSI + ECR pull via node role)
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ...
        StringLike = {
          "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:*:healthcare-app-sa"
        }
      }
    }]
  })
...
resource "aws_iam_role_policy" "app_secrets" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [var.secrets_secret_arn]
    }]
  })
}

resource "aws_iam_role_policy" "app_ecr" {
  name = "${var.cluster_name}-app-ecr"
  role = aws_iam_role.app_irsa.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
